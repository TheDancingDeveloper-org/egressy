...
test('distinguishes protection from unavailable service', async () => {
  ...
  expect(screen.getByRole('heading', { name: 'VPN connection' }).nextElementSibling).toHaveTextContent('unavailable')
  expect(screen.getByRole('heading', { name: 'Internet reachability test' }).nextElementSibling).toHaveTextContent('needs attention')
  expect(screen.getByText(/cannot prove the route inside/)).toBeInTheDocument()
...
test('shows Docker route-intent mismatch without changing compliance', async () => {
  ...
  expect(screen.getByText('incorrect route')).toBeInTheDocument()
