...
test('renders protected-but-unavailable and operational diagnostics', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.getByRole('heading', { name: 'Protection' }).locator('..')).toContainText('enforced')
  await expect(page.getByRole('heading', { name: 'Availability' }).locator('..')).toContainText('unavailable')
  ...
  await expect(page.getByRole('heading', { name: 'External verification' }).locator('..')).toContainText('verified')

  await page.getByRole('link', { name: 'Clients' }).click()
  await expect(page.getByText(/cannot prove an effective/)).toBeVisible()
  ...
  await expect(page.locator('#external-probe')).toContainText('healthy')
